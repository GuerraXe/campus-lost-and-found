...
`./mvnw verify` — **green on Oracle JDK 26**. 43 unit (Surefire) + 43 integration
(Failsafe, full Spring context on Flyway-migrated H2) = **86 tests**.

Integration tests run with `@ActiveProfiles("test")` so the production
`spring.jpa.open-in-view=false` is in effect — a lazy-loading bug in a response mapping
fails a test, not only production.
...
| `ListingIT` | 9 | **creating listings** (verified-email gate, future date → 422, missing fields → field errors, detail response carries attributes + reporter with `open-in-view` off); **redaction** (private details / reporter hidden from anonymous + stranger, shown to owner); **searching & filtering** (q, kind, category, building, date range, empty result); invalid sort field → 400; **authorization** (non-owner patch → 403); **updating status** (illegal transition, REMOVED needs moderator, RECOVERED gated); attributes add/remove + duplicate → 409 |
...
| `SecurityIT` | 7 | defensive headers present; **write works with no CSRF token**; not-found = problem+json, no stack trace; malformed JSON → 400; **wrong HTTP method → 405** (not 500); unknown API route → 404 (not 500); `GET /listings/mine` needs auth |
